...
app_server = FastAPI(title="NaRuTagAI API", version="1.3.2",
                     description="This API server is intended for the NaRuTagAI project. For rights, contact the service owner.")
...
@app_server.get("/inference/", tags=["Main"])
async def inference(url: str = Query(..., description="Encoded Rutube video URL")):
    """
    Route for predicting tags, category, and subcategory from a video URL.

    :param url: URL of the video, passed as a query parameter.
    :return: Response model in JSON format.
    """
    try:
        # Decode the URL parameter to the original URL
        decoded_url = unquote(url)
        # Call the prediction service using the decoded URL
        result = main_services.predict(decoded_url)
        return result
