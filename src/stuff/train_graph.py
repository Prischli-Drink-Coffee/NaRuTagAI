...
class Graduate:
    ...
    def __post_init__(self):
        ...
        self.path_to_graph = Path(os.path.join(project_path, self.path_to_data, "graph.pt"))
    ...
    def get_loaders(self):
        # Инициализируем граф
        self.graph_reader = GraphReader(self.path_to_graph)
        self.graph = self.graph_reader.load_graph()
        input_nodes = torch.nonzero(self.graph.label_mask)[:, 0]
        ...
        for sampled_data in tqdm(loader, desc="Sample data"):
            ...
            self.list_loader.append(sampled_data)

    def get_model(self):
        # Инициализируем модель
        self.model = CustomClassifierWithGAT().to(self.device)
